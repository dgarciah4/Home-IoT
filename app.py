...
def home ():
    if request.method == 'POST':
        ...
        ## TEMP UP
        elif request.form.get('tempUp') == 'Temp Up':
            temp_Up()
            interTemp = get_internal_temp()
            extTemp = get_currentWeather()
            return render_template('home.html', extTemp=extTemp, interTemp=interTemp)
        ...
        ## TEMP DOWN
        elif request.form.get('tempDown') == 'Temp Down':
            temp_Down()
            interTemp = get_internal_temp()
            extTemp = get_currentWeather()
            return render_template('home.html', extTemp=extTemp, interTemp=interTemp)
...
def graphs ():
    ...
    """
    df = pd.DataFrame() 
    df = pd.read_csv('electric.txt', header = None, delim_whitespace=True)
    df.index = df.index + 1
    df.columns = ["Day", "Usage_Total", "Cost_Total"]


    x,y = df['Day'], df['Usage_Total']
    ...
    Y = df['Usage_Total'].values
    values = np.append(Y, regr.predict(Y.reshape(-1,1)) )
    ...
    return render_template ('graphs.html', title='Monthly Usages', max=2000000, elecUsage=elecUsage, elecCost=elecCost, waterCost=waterCost, waterUsage=waterUsage, heaterUsage=heaterUsage, heaterCost=heaterCost, colors=colors, labels=bar_labels, values=bar_values, line_values=line_values, i = i, wC = wC, line_label = x)
